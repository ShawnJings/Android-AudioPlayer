#include "com_timeapp_shawn_voiceplay_opensles_SoundTrackController.h"
...
JNIEXPORT jboolean JNICALL Java_com_timeapp_shawn_voiceplay_opensles_SoundTrackController_setAudioDataSource(JNIEnv * env, jobject obj, jstring accompany, jfloat percent) {
	const char* accompanyPath = env->GetStringUTFChars(accompany, NULL);
    LOGE("SoundService::GetInstance() begin");
    ...
    LOGE("SoundService::initSongDecoder() begin");
	//设置播放文件路径以及解码buffer的1秒钟的百分比
	soundService->initSongDecoder(accompanyPath, percent);

    LOGE("SoundService::initSoundTrack() begin");
...
JNIEXPORT jint JNICALL Java_com_timeapp_shawn_voiceplay_opensles_SoundTrackController_getAccompanySampleRate(JNIEnv * env, jobject obj) {
...
JNIEXPORT void JNICALL Java_com_timeapp_shawn_voiceplay_opensles_SoundTrackController_play
...
JNIEXPORT jint JNICALL Java_com_timeapp_shawn_voiceplay_opensles_SoundTrackController_getCurrentTimeMills(JNIEnv * env, jobject obj) {
	if (NULL != soundService) {
...
JNIEXPORT void JNICALL Java_com_timeapp_shawn_voiceplay_opensles_SoundTrackController_stop
(JNIEnv * env, jobject obj) {
	if (NULL != soundService) {
		soundService->stop();
		soundService = NULL;
