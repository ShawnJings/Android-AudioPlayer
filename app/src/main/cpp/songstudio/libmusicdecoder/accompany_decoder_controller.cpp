...
void* AccompanyDecoderController::startDecoderThread(void* ptr) {
    ...
    LOGI("enter AccompanyDecoderController::xxx");
    LOGI("Control reaches end of non-void function enter AccompanyDecoderController::startDecoderThread ending");
    return NULL;
}
...
void AccompanyDecoderController::initDecoderThread() {
	...
	int val = pthread_create(&songDecoderThread, NULL, startDecoderThread, this);
	LOGE("initDecoderThread VAL = %d", val);
}
...
void AccompanyDecoderController::destroy() {
	...
	if (NULL != accompanyDecoder) {
        LOGI("AccompanyDecoderController accompanyDecoder destroy");
